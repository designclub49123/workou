              ...
              <div className="flex flex-col items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-2">
                    Your activities will appear here as you use the app. Start by applying to jobs or updating your profile.
                  </p>
                </div>
              </div>
            </CardContent>
          ...
          <div className="space-y-6">
            {Object.entries(groupedActivities).map(([date, dayActivities]) => (
              <div key={date} className="space-y-3">
                <h3 className="text-sm font-medium text-muted-foreground sticky top-0 bg-background py-2" role="heading" aria-level={3}>
                  {getDateLabel(date)}
                </h3>
                <div className="space-y-3 relative">
                  ...
                  {dayActivities.map((activity, index) => (
                    <div key={activity.id} className="flex gap-3 sm:gap-4 relative" role="article" aria-label={activity.title}>
                      {/* Icon */}
                      <div className={`flex-shrink-0 h-10 w-10 rounded-full border flex items-center justify-center ${getActivityColor(activity.activity_type)} z-10 bg-background`} aria-hidden="true">
                        ...
                        <Card className="border-border hover:shadow-md transition-shadow duration-200">
                          <CardContent className="p-3 sm:p-4">
                            <div className="space-y-1">
                              ...
                              <p className="text-xs text-muted-foreground" title={new Date(activity.created_at).toLocaleString()}>
